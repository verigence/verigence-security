...
def _seed_users_and_tenant(engine, *, tenant_status: str):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                ...
                VALUES (:tenant_id,:code,:code,:status,:now,:now)
                """
            ),
            {
                "tenant_id": tenant_id,
                "code": f"role-{tenant_id}",
                "status": tenant_status,
                "now": now,
            },
        )
    return actor_id, user_id, tenant_id


def _cleanup(engine, actor_id: str, user_id: str, tenant_id: str) -> None:
    with engine.begin() as conn:
        conn.execute(
            text(
...
def test_configuring_tenant_can_receive_operating_role_for_uc02_setup() -> None:
    assert TEST_DATABASE_URL is not None
    engine = create_engine(_sqlalchemy_url(TEST_DATABASE_URL), pool_pre_ping=True)
    actor_id, user_id, tenant_id = _seed_users_and_tenant(
        engine,
        tenant_status="CONFIGURING",
    )
    try:
        ...
        assert result.role_key == "PC"
    finally:
        _cleanup(engine, actor_id, user_id, tenant_id)
        engine.dispose()


def test_suspended_tenant_cannot_receive_new_operating_role() -> None:
    assert TEST_DATABASE_URL is not None
    engine = create_engine(_sqlalchemy_url(TEST_DATABASE_URL), pool_pre_ping=True)
    actor_id, user_id, tenant_id = _seed_users_and_tenant(
        engine,
    ...
    try:
        with Session(engine) as session, pytest.raises(
            ValueError,
            match="Tenant must be CONFIGURING or ACTIVE for an operating-role assignment",
        ):
