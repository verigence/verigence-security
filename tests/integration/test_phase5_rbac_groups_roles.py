...
def _url(url: str) -> str:
    ...
    if url.startswith("postgresql+asyncpg://"):
        suffix = url.removeprefix("postgresql+asyncpg://")
        return f"postgresql+psycopg://{suffix}"
    raise ValueError("TEST_DATABASE_URL must be PostgreSQL")


def _seed(engine: object, tenant_id: str, users: tuple[str, str], permission: str) -> None:
    now = datetime.now(UTC)
    actor_id, member_id = users
...
def test_group_and_direct_roles_drive_effective_rbac_and_versions() -> None:
    ...
    _seed(engine, tenant_id, (actor_id, member_id), permission)
    ...
    try:
        with Session(engine) as session:
            ...
            with pytest.raises(SecurityError, match="ROLE_REQUIRED"):
