...
6. The existing DEV schema is adopted through `0024_uc02_tenant_hard_delete.sql`; the initial 0001-0024 ledger adoption is written atomically in one database transaction, so partial historical adoption cannot occur.
7. Subsequent migrations are executed only when absent from the ledger.
8. Fresh databases execute `0001` onward normally and record each successful migration.
9. Deployment continues to run the existing post-migration Security invariants before building or attaching a Railway image.
